...
video_put_args = reqparse.RequestParser()  # validate the input data
...
class Video(Resource):
    ...
    @marshal_with(rescource_fields) #wraps around the instance we get from the database and then converts into json
    ...
    def patch(self,video_id):
        ...
        result = VideoModel.query.filter_by(id=video_id).first()
        if not result:
            abort(409, message="Video not available")
        if args["name"]:
